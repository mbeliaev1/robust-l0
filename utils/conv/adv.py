...
class adv_trainer():
    def __init__(self,
        ...
        self.mu, self.sigma = mu_sigma(self.beta, CIFAR=True)
        ...
        # We will create optimizers later based on iteration and learning rate
    ...
    def test(self):
        '''
        Preforms a test on self.net using the CIFAR test dataset.
    ...
    def attack_save(self):
        ...
        for x,y,adv_x,adv_y in zip(self.Data['x_og'],self.Data['y_og'],adv_xs,adv_ys):
            ...
            rem_x, rem_y = torch.cat((rem_x,torch.clone(new_x[self.Data['bs']:-1]))),torch.cat((rem_y,torch.clone(new_y[self.Data['bs']:-1])))
    ...
    def run(self):
        ...
        while self.iter<self.num_iters:
            res_str = "Running iter%d, with lr: %.3f"%(self.iter,self.lrs[self.iter])
            print(res_str)
